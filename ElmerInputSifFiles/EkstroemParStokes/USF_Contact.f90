...
! means have the possibility to change
  ...
  Fwater = 0.0_dp
  Fbase = 0.0_dp
      !write(*,*) 'BEFORE Test Tolerance', thresh
      !write(*,*) 'relChange', relChange
      !write(*,*) 'relChangeOld', relChangeOld
      !write(*,*) 'relchange', relchange
      !write(*,*) 'TestContact', TestContact
      !write(*,*) 'yesChange', yesChange
      !write(*,*) 'GLmoves', GLmoves
  IF ( (relChange.NE.relChangeOld) .AND. (relchange.GT.0.0_dp) .AND. &
     ...
     DO tt = 1, Model % NumberOfBoundaryElements
        ...
        DO ii = 1,n
          ...
          !comment
           !write(*,*) 'Before load and reaction comparison'
           ...
           !write(*,*) 'After load and reaction comparison'

           ! comparison between water pressure and bed action
           comp = ABS( SUM( Fwater * Normal ) ) - ABS( SUM( Fbase * Normal ) )
           !write(*,*) 'After comparison between water pressure and bed action'
           !write(*,*), 'This is comp value', comp
           !write(*,*), 'This is Normal value', Normal
           !write(*,*), 'This is Fwater value', Fwater
           !write(*,*), 'This is Fbase value', Fbase

           IF (comp .GE. 0.0_dp) THEN
               !write(*,*), 'In Comparison Loop'
              IF (thresh.LE.0.0_dp) THEN
                 !write(*,*), 'In thresh loop'
                 GroundedMask(Nn) = -1.0_dp
                 GL_retreat = GL_retreat + 1
                 !write(*,*) 'WRON IF RD comp, THRESH: ', comp, thresh,  Distance(DistancePerm(Element % NodeIndexes(ii)))
                 PRINT *, 'REtreat of the Grounding Line : '
                 PRINT *, Nodes % x(ii), Nodes % y(ii), Nodes % z(ii)
              ELSE
                 write(*,*) 'Distance, thresh',  Distance(DistancePerm(Element % NodeIndexes(ii))),thresh
                 IF ( Distance(DistancePerm(Element % NodeIndexes(ii))).LE.thresh ) THEN
                    GroundedMask(Nn) = -1.0_dp
                    GL_retreat = GL_retreat + 1
                    write(*,*) 'RD comp, THRESH: ', comp, thresh, Distance(DistancePerm(Element % NodeIndexes(ii)))
                    PRINT *, 'RETREAT of the Grounding Line : '
                    PRINT *, Nodes % x(ii), Nodes % y(ii), Nodes % z(ii)
                 END IF
              END IF
     ...
     END IF
  END IF
    !write(*,*) 'AFTER Test Tolerance', thresh
  ...
  ! for the bottom surface, where the GroundedMask is defined
    ...
    !write(*,*) 'GroundedMaskPerm', GroundedMaskPerm(nodenumber)
  ...
  ! If GLtype = Discontinuous -> Bdrag = 0 if at one node of the element Mask<=0
     ...
     IF (Friction) THEN
        ...
        CASE ('weertman')
           !write(*,*) 'In WEERTMAN'
           !write(*,*) 'In WEERTMAN', nodenumber
           !write(*,*) 'In WEERTMAN', y
           Bdrag = Sliding_weertman(Model, nodenumber, y)
           !write(*,*) 'After WEERTMAN'
        CASE ('budd')
